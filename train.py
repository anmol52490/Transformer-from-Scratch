...
def get_ds(config):
    ...
    print(f'Maximum length of target sentence is {max_len_trg}')
...
def get_model(config, vocab_src_len, vocab_trg_len):
    model = build_transformer(vocab_src_len, vocab_trg_len, config['seq_len'], config['seq_len'] )
    return model
...
def train_model(config):
    ...
    for epoch in range(initial_epoch, config['num_epochs']):
        ...
        for batch in batch_iterator:
            ...
            # A note The  CEL expects it to be logits.shape = (N,C) and targets.shape(C) 
            # where N = no of training examples, C = number of classes.
            loss = loss_fn(proj_output.view(-1, tokenizer_trg.get_vocab_size()), label.view(-1)) # (1)
